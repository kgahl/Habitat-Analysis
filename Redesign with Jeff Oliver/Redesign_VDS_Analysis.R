...
#Read in data file with all combinations 
All_Variables <- read.csv("Redesign with Jeff Oliver/VelocityDepthSubstrate_Variables.csv")

#Output dataframe to write to file
All_Rows <- NULL

#Iterate over each row of All Variables
for (row_i in 1:nrow(All_Variables)) {
 #Extract abbreviations for output
  Species_Abb <- All_Variables$Species_Abb[row_i]
  Stream_Abb <- All_Variables$Stream_Abb[row_i]
  Season_Abb <- All_Variables$Season_Abb[row_i]
  Mesohabitat_Abb <- All_Variables$Mesohabitat_Abb[row_i]
  
  #Extract values for filtering 
  Species_Name <- All_Variables$Species[row_i]
  Stream_Name <- All_Variables$Stream[row_i]
  Season_Months <- strsplit(x = All_Variables$Season[row_i], split = ",")[[1]]
  Mesohabitat_Name <- All_Variables$Mesohabitat[row_i]
  ...
  #Only do resampling when numer of individuals is great than 10
  ...
  if (nrow(Storage) >1) {
    Storage_Means$Velocity_Lower <- quantile(Storage$Velocity, probs = 0.025)
    Storage_Means$Velocity_Upper <- quantile(Storage$Velocity, probs = 0.975)
    
    ######FINISH FILLING OUT THIS SECTION
        #Also compare to previous results (more than and less than 10)
        #change to 100000
  } 
...
#Output to File
write.csv(x = All_Rows, file = "RedesignVDSOutput.csv")
################################################################################

...
##Read In
#All data available
NotAsFishy <- read.csv("~/School/Bonar Lab/Project/Analysis/Habitat - Telemetry/Habitat-Analysis/Output/NotAsFishy.csv")

#### ####
# BeaverOcc <- NotAsFishy %>%           #195                                             
#   filter(Stream == 'Beaver')%>%          
#   filter(Presence == 1)          
# 
# ClearOcc <- NotAsFishy %>%           #181                                             
#   filter(Stream == 'Clear')%>%          
#   filter(Presence == 1) 
# 
# 
# BeaverAvail <- NotAsFishy %>%           #936                                             
#   filter(Stream == 'Beaver')%>%          
#   filter(Presence == 0)                                                #0 = Available
# BeaverRiff <- BeaverAvail %>%           #383
#   filter(Mesohabitat == 'Riffle')
# BeaverRun <- BeaverAvail %>%            #149
#   filter(Mesohabitat == 'Run')
# BeaverPool <- BeaverAvail %>%           #404
#   filter(Mesohabitat == 'Pool')
# 
# NABeav <- na.omit(BeaverAvail$Presence)  #936
# NABeav <- length(NABeav[!is.na(NABeav)]) #936
# 
# RiffBeav <- na.omit(BeaverRiff$Presence)  #383
# RiffBeav <- length(RiffBeav[!is.na(RiffBeav)]) #383
# 
# ClearAvail <- NotAsFishy %>%           #1312                                             
#   filter(Stream == 'Clear')%>%          
#   filter(Presence == 0)                                                #0 = Available
# ClearRiff <- ClearAvail %>%            #712
#   filter(Mesohabitat == 'Riffle')
# ClearRun <- ClearAvail %>%             #113
#   filter(Mesohabitat == 'Run')
# ClearPool <- ClearAvail %>%            #487
#   filter(Mesohabitat == 'Pool')
# 
# 
#### ####
...
Variable_Combos <- read.csv("~/School/Bonar Lab/Project/Analysis/Habitat - Telemetry/Habitat-Analysis/Means_Variables.csv")

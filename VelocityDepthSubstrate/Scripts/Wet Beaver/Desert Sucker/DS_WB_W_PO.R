...
########## Only 6 Observations and 6 Unique ######################
...
##### DS_WB_W_PO #####
##Desert Sucker Wet Beaver Creek Spawning (December - February) Pool Data 
...
#Only Spawning period- December - February and Pools                         #[6 desert sucker observations, 112 available]
  DS_WB_W_PO <- DS_AV_WB %>%
...
##Number of Unique Individuals Observed
  Unique_Fish <- na.omit(unique(DS_WB_W_PO$Tag))                               #Gives number of individuals- doesn't count the same fish more than once
...
##Find Min and Max of variables used by fish
  ...
  MinMax <- data.frame(DataSet = "DS_WB_W_PO", Velocity_Min, Velocity_Max, Depth_Min, 
                       Depth_Max, Substrate_Min, Substrate_Max)
  
  

#Do some quick data exploration
  boxplot(Velocity ~ Month, DS_WB_W_PO)
  boxplot(Substrate ~ Month, DS_WB_W_PO)
  boxplot(Depth ~ Month, DS_WB_W_PO)

#Standardize and weight
  #Add weight column to make binomial GLM equivalent to Inhomogeneous Poisson Point Process Model
  DS_WB_W_PO$weight <- ifelse(DS_WB_W_PO$Presence == 1, 1, 5000)
  
  #Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) DS_WB_W_PO
  mean_velocity  <- mean(DS_WB_W_PO$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covaPOates or comparing models each containing different covariates it is helpful to 
  sd_velocity    <- sd(DS_WB_W_PO$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
  mean_substrate <- mean(DS_WB_W_PO$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
  sd_substrate   <- sd(DS_WB_W_PO$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
  mean_depth     <- mean(DS_WB_W_PO$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
  sd_depth       <- sd(DS_WB_W_PO$Depth,na.rm = T)                               #outside of the i-loop (DS_WB_W_PO) and then using this to do the rescaling inside the i-loop (DS_WB_W_PO) 

  #Rescale your covariates
  DS_WB_W_PO$zSubstrate <- (DS_WB_W_PO$Substrate - mean_substrate)/sd_substrate
  DS_WB_W_PO$zVelocity <- (DS_WB_W_PO$Velocity - mean_velocity)/sd_velocity
  DS_WB_W_PO$zDepth <- (DS_WB_W_PO$Depth - mean_depth)/sd_depth
  
  
###RSF  
  ##General Linear Modeling
  #Fit the model
  Model_Vel <- glm(Presence ~ zVelocity, data = DS_WB_W_PO, family = binomial,weights = weight)             #family specifies the details of the model (GLM) used.  
  Model_Subs <- glm(Presence ~ zSubstrate, data = DS_WB_W_PO, family = binomial,weights = weight)           #one covariate: substrate
  Model_Depth <- glm(Presence ~ zDepth, data = DS_WB_W_PO, family = binomial,weights = weight)
  Model_Full <- glm(Presence ~ zVelocity + zDepth, data = DS_WB_W_PO, family=binomial,weights = weight)  #model with two covariates (velocity, depth); because there are only 19 observations we can't do all 3 (10/ rule of thumb)
  ...
  ##### HSC #####
  HSC_data <- filter(DS_WB_W_PO, Presence == 1)                                   #isolate occupied site data
  Avail_Data <- filter(DS_WB_W_PO, Presence == 0)

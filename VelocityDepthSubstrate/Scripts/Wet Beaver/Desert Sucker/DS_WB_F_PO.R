...
##### DS_WB_F_PO #####
##Desert Sucker Wet Beaver Creek Spawning (September - November) Pool Data 
...
#Only Spawning period- September - November and Pools                         #[11 desert sucker observations,  128 available]
DS_WB_F_PO <- DS_AV_WB %>%
...
Unique_Fish <- na.omit(unique(DS_WB_F_PO$Tag))                               #Gives number of individuals- doesn't count the same fish more than once
...
MinMax <- data.frame(DataSet = "DS_WB_F_PO", Velocity_Min, Velocity_Max, Depth_Min, 
                     Depth_Max, Substrate_Min, Substrate_Max)


#Do some quick data exploration
boxplot(Velocity ~ Month, DS_WB_F_PO)
boxplot(Substrate ~ Month, DS_WB_F_PO)
boxplot(Depth ~ Month, DS_WB_F_PO)

#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i
mean_velocity  <- mean(DS_WB_F_PO$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covaPOates or comparing models each containing different covariates it is helpful to 
sd_velocity    <- sd(DS_WB_F_PO$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
mean_substrate <- mean(DS_WB_F_PO$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
sd_substrate   <- sd(DS_WB_F_PO$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
mean_depth     <- mean(DS_WB_F_PO$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
sd_depth       <- sd(DS_WB_F_PO$Depth,na.rm = T)                               #outside of the i-loop (DS_WB_F_PO) and then using this to do the rescaling inside the i-loop (Data_i) 
...
Unique_Fish <- na.omit(unique(DS_WB_F_PO$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  #Sample Used data (your_full_data ->DS_WB_F_PO<- should be for one species, one site, one season, and (maybe) one mesohabitat) #The maybe will come into play when I look at the n\NumberOfIndividuals for each species/site/season/meso combo. If there are any with less than 10 unique individuals then I should combine mesohabs.
  
  Used_Data_i <- NULL
  for(j in 1:Number_Of_Individuals){                                         #create empty data frame
    #Ditto above on the na.omi(); it is good practice
    Fish_j <- na.omit(unique(DS_WB_F_PO$Tag))[j]                   #randomly select one tag number, label it Fish j
    Fish_js_Rows <- DS_WB_F_PO[which(DS_WB_F_PO$Tag==Fish_j),]    #grabs Fish j's rows of data
  ...
  Avail_Data <- DS_WB_F_PO[which(DS_WB_F_PO$Presence == 0), ]               #Separate availabile data. y is Presence
...
View(Storage)
write_csv(Storage, file = "Output/DS_WB_F_PO_Results.csv")
...
#0.01202497                         #selecting fast water (positive)
...
#0.000529969                          #favoring large substrate but just barely

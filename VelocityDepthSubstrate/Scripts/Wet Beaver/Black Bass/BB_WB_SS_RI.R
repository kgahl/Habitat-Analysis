...
#Only Spawning period- April-June and Riffles                                 #[3 Black Bass observations, 116 available]
BB_WB_SS_RI <- BB_AV_WB %>%
...
MinMax <- data.frame(DataSet = "BB_WB_SS_RI", Velocity_Min, Velocity_Max, Depth_Min, 
                     Depth_Max, Substrate_Min, Substrate_Max)


#Do some quick data exploration
boxplot(Velocity ~ Month, BB_WB_SS_RI)
boxplot(Substrate ~ Month, BB_WB_SS_RI)
boxplot(Depth ~ Month, BB_WB_SS_RI)

#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i
mean_velocity  <- mean(BB_WB_SS_RI$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
sd_velocity    <- sd(BB_WB_SS_RI$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
mean_substrate <- mean(BB_WB_SS_RI$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
sd_substrate   <- sd(BB_WB_SS_RI$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
mean_depth     <- mean(BB_WB_SS_RI$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
sd_depth       <- sd(BB_WB_SS_RI$Depth,na.rm = T)                               #outside of the i-loop (BB_WB_SS_RI) and then using this to do the rescaling inside the i-loop (Data_i) 
...
Unique_Fish <- na.omit(unique(BB_WB_SS_RI$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)

Number_Of_Individuals <- length(Unique_Fish[!is.na(Unique_Fish)])             #removes NA 
length(Unique_Fish) #3
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  #Sample Used data (your_full_data ->BB_WB_SS_RI<- should be for one species, one site, one season, and (maybe) one mesohabitat) #The maybe will come into play when I look at the n\NumberOfIndividuals for each species/site/season/meso combo. If there are any with less than 10 unique individuals then I should combine mesohabs.
  
  Used_Data_i <- NULL
  for(j in 1:Number_Of_Individuals){                                         #create empty data frame
    #Ditto above on the na.omi(); it is good practice
    Fish_j <- na.omit(unique(BB_WB_SS_RI$Tag))[j]                   #randomly select one tag number, label it Fish j
    Fish_js_Rows <- BB_WB_SS_RI[which(BB_WB_SS_RI$Tag==Fish_j),]    #grabs Fish j's rows of data
  ...
  Avail_Data <- BB_WB_SS_RI[which(BB_WB_SS_RI$Presence == 0), ]               #Separate availabile data. y is Presence
...
write_csv(Storage, file = "Output/BB_WB_SS_RI_Results.csv")

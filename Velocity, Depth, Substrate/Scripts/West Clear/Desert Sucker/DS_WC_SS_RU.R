...
#Only Spawning period- Mar-June and Runs                                   #[7 desert sucker observations, 64 available]
  DS_WC_SS_RU <- DS_AV_WC %>%
...
##Number of Unique Individuals Observed
  Unique_Fish <- na.omit(unique(DS_WC_SS_RU$Tag))                               #Gives number of individuals- doesn't count the same fish more than once
  
  Number_Of_Individuals <- length(Unique_Fish[!is.na(Unique_Fish)])             #removes NA 
  length(Unique_Fish)  #6
...
##Find Min and Max of variables used by fish
  ...
  MinMax <- data.frame(DataSet = "DS_WC_SS_RU", Velocity_Min, Velocity_Max, Depth_Min, 
                       Depth_Max, Substrate_Min, Substrate_Max)
  
  
#Do some quick data exploration
  boxplot(Velocity ~ Month, DS_WC_SS_RU)
  boxplot(Substrate ~ Month, DS_WC_SS_RU)
  boxplot(Depth ~ Month, DS_WC_SS_RU)
  
#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i
  mean_velocity  <- mean(DS_WC_SS_RU$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
  sd_velocity    <- sd(DS_WC_SS_RU$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
  mean_substrate <- mean(DS_WC_SS_RU$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
  sd_substrate   <- sd(DS_WC_SS_RU$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
  mean_depth     <- mean(DS_WC_SS_RU$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
  sd_depth       <- sd(DS_WC_SS_RU$Depth,na.rm = T)                               #outside of the i-loop (DS_WC_SS_RU) and then using this to do the rescaling inside the i-loop (Data_i) 
  ...
  for(i in 1:N_reps){                                                            #do 10,000 reps
    ...
    #Sample Used data (your_full_data ->DS_WC_SS_RU<- should be for one species, one site, one season, and (maybe) one mesohabitat) #The maybe will come into play when I look at the n\NumberOfIndividuals for each species/site/season/meso combo. If there are any with less than 10 unique individuals then I should combine mesohabs.
    
    Used_Data_i <- NULL
    for(j in 1:Number_Of_Individuals){                                         #create empty data frame
      #Ditto above on the na.omi(); it is good practice
      Fish_j <- na.omit(unique(DS_WC_SS_RU$Tag))[j]                   #randomly select one tag number, label it Fish j
      Fish_js_Rows <- DS_WC_SS_RU[which(DS_WC_SS_RU$Tag==Fish_j),]    #grabs Fish j's rows of data
    ...
    Avail_Data <- DS_WC_SS_RU[which(DS_WC_SS_RU$Presence == 0), ]               #Separate availabile data. y is Presence
  ...
  View(Storage)  
  write_csv(Storage, file = "Output/DS_WC_SS_RU_Results.csv")
  ...
  #0.04675968                         #selecting fast water 

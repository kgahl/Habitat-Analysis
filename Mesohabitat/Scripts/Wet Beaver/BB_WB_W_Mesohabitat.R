#BB WB W Mesohabitat
#Black Bass on Wet Beaver in Winter Months for Mesohabitat selection
...
##### BB_WB_W #####
##Black Bass Beaver Creek Spawning (Sept - November) 
...
#Only Spawning period- September - November and Riffles                       #[33 Black Bass observations, 310 available]
  BB_WB_W <- BB_AV_WB %>%
    filter(Month %in% c('December', 'January', 'February'))
  remove(BB_AV_WB)

#Make R read Mesohabitat as a three-level categorical factor
  BB_WB_W$Mesohabitat <- as.factor(BB_WB_W$Mesohabitat)


#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i

# mean_velocity  <- mean(BB_WB_F$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
# sd_velocity    <- sd(BB_WB_F$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and
# mean_substrate <- mean(BB_WB_F$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that
# sd_substrate   <- sd(BB_WB_F$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each
# mean_depth     <- mean(BB_WB_F$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set
# sd_depth       <- sd(BB_WB_F$Depth,na.rm = T)                               #outside of the i-loop (BB_WB_F) and then using this to do the rescaling inside the i-loop (Data_i)
...
# Depth_full = NA)
  ...
  Unique_Fish <- na.omit(unique(BB_WB_W$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  for(j in 1:Number_Of_Individuals){                                            #create empty data frame
    Fish_j <- na.omit(unique(BB_WB_W$Tag))[j]                                   #randomly select one tag number, label it Fish j
    Fish_js_Rows <- BB_WB_W[which(BB_WB_W$Tag==Fish_j),]                        #grabs Fish j's rows of data
...
##### Selection Ratios
    ...
    Avail_Data <- BB_WB_W[which(BB_WB_W$Presence == 0), ]                       #Separate available data.
...
View(Storage)
write_csv(Storage, file = "Output/BB_WB_WMesohabitatResults.csv")
...
#and riffles and pools and runs).
 ...
 #Riffle Mesohabitat
  ...
  #-1.304391                         # avoiding riffles more strongly than pools         
  quantile(Storage$Mesohabitat_Riffle,c(0.025,0.975), na.rm = TRUE) #2.5%  to  97.5%      
  #-2.3333568 -0.6286086                #significant                        #Significance determined by crossing of 0           
  
  
 #Run Mesohabitat
  hist(Storage$Mesohabitat_Run)                                                        
  mean(rep(Storage$Mesohabitat_Run), na.rm = TRUE)                                           
  #-0.08701767                         #not/selecting for run more strongly than pools
  quantile(Storage$Mesohabitat_Run,c(0.025,0.975), na.rm = TRUE) #2.5%  to  97.5%      
  #-0.4094255  0.1967103                 #not significant               

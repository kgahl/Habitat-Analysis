...
##### DS_WC_SS #####
##Desert Sucker West Clear Creek Spawning (Sept - November) Riffle Data 

#Combine all Desert Sucker and available data on WB <----- DS_AV_WC
...
DS_AV_WC <- rbind(DS_WC, AV_WC)                                               #combine BB and available
remove(DS_WC, AV_WC)

#Only Spawning period- September - November and Riffles                       #[17 Desert Sucker observations, 807 available]
DS_WC_SS <- DS_AV_WC %>%
  filter(Month %in% c('April','May','June','July'))
remove(DS_AV_WC)

DS_WC_SS$Mesohabitat <- as.factor(DS_WC_SS$Mesohabitat)


#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i

   # mean_velocity  <- mean(DS_WC_SS$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
  # sd_velocity    <- sd(DS_WC_SS$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and
  # mean_substrate <- mean(DS_WC_SS$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that
  # sd_substrate   <- sd(DS_WC_SS$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each
  # mean_depth     <- mean(DS_WC_SS$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set
  # sd_depth       <- sd(DS_WC_SS$Depth,na.rm = T)                               #outside of the i-loop (DS_WC_SS) and then using this to do the rescaling inside the i-loop (Data_i)
...
#Create data frame to store coefficients
  ...
  Unique_Fish <- na.omit(unique(DS_WC_SS$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  Avail_Data <- DS_WC_SS[which(DS_WC_SS$Presence == 0), ]                       #Separate availabile data. y is Presence
  ...
  #Run
  Avail_Prob_Run <- sum(na.omit(Avail_Data$Mesohabitat== 'Run')/nrow(Avail_Data)) #0.08152866
  Used_Prob_Run<- sum(na.omit(Used_Data_i$Mesohabitat== 'Run')/nrow(Used_Data_i))  #0.1538462 
...
View(Storage)
write_csv(Storage, file = "~/Kaitlyn's Data Analysis/Analysis/Data Ready for HSC/Data-Ready-for-HSC/Output/Storage Results/DS_WC_SSMesohabitatResults.csv")
...
#Riffle Mesohabitat
  ...
  #-0.6761771    1.8087296                 #not significant#Significance determined by crossing of 0
...
#Run Mesohabitat
  ...
  #-14.336697     2.682732                 #not significant               

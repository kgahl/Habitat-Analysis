...
BB_WB_F$Mesohabitat <- as.factor(BB_WB_F$Mesohabitat)
...
#Create data frame to store coefficients
  ...
  Unique_Fish <- na.omit(unique(BB_WB_F$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  #Sample Used data (your_full_data ->BB_WB_F<- should be for one species, one site, one season, and (maybe) one mesohabitat) #The maybe will come into play when I look at the n\NumberOfIndividuals for each species/site/season/meso combo. If there are any with less than 10 unique individuals then I should combine mesohabs.
  
  Used_Data_i <- NULL
  for(j in 1:Number_Of_Individuals){                                         #create empty data frame
    #Ditto above on the na.omi(); it is good practice
    Fish_j <- na.omit(unique(BB_WB_F$Tag))[j]                   #randomly select one tag number, label it Fish j
    Fish_js_Rows <- BB_WB_F[which(BB_WB_F$Tag==Fish_j),]    #grabs Fish j's rows of data
  ...
  Avail_Data <- BB_WB_F[which(BB_WB_F$Presence == 0), ]                       #Separate availabile data. y is Presence
...
View(Storage)
write_csv(Storage, file = "Output/BB_WB_F_MesohabitatResults.csv")
...
#Riffle Mesohabitat
  ...
  #-1.68899964  0.01574835                #not significant#Significance determined by crossing of 0
...
#Run Mesohabitat
  ...
  #-1.68899964  0.01574835                #not significant               

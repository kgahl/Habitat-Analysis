...
##### BB_WB_F_PO #####
##Black Bass West Beaver Creek Spawning (Sept - Nov) Pool Data 
...
#Only Spawning period- Sept - Nov and Pools                                 #[19 Black Bass observations, 128  available]
BB_WB_F_PO <- BB_AV_WB %>%
...
MinMax <- data.frame(DataSet = "BB_WB_F_PO", Velocity_Min, Velocity_Max, Depth_Min, 
                     Depth_Max, Substrate_Min, Substrate_Max)


#Do some quick data exploration
boxplot(Velocity ~ Month, BB_WB_F_PO)
boxplot(Substrate ~ Month, BB_WB_F_PO)
boxplot(Depth ~ Month, BB_WB_F_PO)

#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i
mean_velocity  <- mean(BB_WB_F_PO$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
sd_velocity    <- sd(BB_WB_F_PO$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
mean_substrate <- mean(BB_WB_F_PO$Substrate,na.rm=T)                           #put them on the same scale such that each covaPOate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
sd_substrate   <- sd(BB_WB_F_PO$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
mean_depth     <- mean(BB_WB_F_PO$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
sd_depth       <- sd(BB_WB_F_PO$Depth,na.rm = T)                               #outside of the i-loop (BB_WB_F_PO) and then using this to do the rescaling inside the i-loop (Data_i) 
...
Unique_Fish <- na.omit(unique(BB_WB_F_PO$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  Avail_Data <- BB_WB_F_PO[which(BB_WB_F_PO$Presence == 0), ]               #Separate availabile data. y is Presence
...
View(Storage)
write_csv(Storage, file = "Output/BB_WB_F_PO_Results.csv")
...
##### Results of the RSFs #####
  ...
  #0.173239                         #selecting fast water (positive)

...
##### DS_WC_SP_RU #####
##Desert Sucker West Clear Creek Spawning (April-July) Run Data 

#Combine all Desert Sucker and available data on WC <----- DS_AV_WC
DS_WC <- NotAsFishy %>%                                                       #separate Clear Creek Desert Sucker
...
DS_AV_WC <- rbind(DS_WC, AV_WC)                                               #combine DS and available
remove(DS_WC, AV_WC)

#Only Spawning period- April-July and Runs                                      #[7 Desert Sucker observations, 64 available]
#None in May    
DS_WC_SP_RU <- DS_AV_WC %>%
  filter(Month %in% c('April', 'May', 'June', 'July')) %>%
  filter(Mesohabitat == 'Run')                                                  # 2 = run  
remove(DS_AV_WC)

#Do some quick data exploration
boxplot(Velocity ~ Month, DS_WC_SP_RU)
boxplot(Substrate ~ Month, DS_WC_SP_RU)
boxplot(Depth ~ Month, DS_WC_SP_RU)

#Calculate mean and SD of each covariate within data subset to be used later for re-scaling (standardizing) Data_i
mean_velocity  <- mean(DS_WC_SP_RU$Velocity,na.rm = T)                        #When fitting regression (GLM) models with multiple covariates or comparing models each containing different covariates it is helpful to 
sd_velocity    <- sd(DS_WC_SP_RU$Velocity,na.rm = T)                            #z-score standardize your covariates. This rescales everything to take covariates that were on different units (meters/percent) and 
mean_substrate <- mean(DS_WC_SP_RU$Substrate,na.rm=T)                           #put them on the same scale such that each covariate will have mean = 0 and SD = 1. Making beta coefficients directly comparable so that 
sd_substrate   <- sd(DS_WC_SP_RU$Substrate,na.rm=T)                             #a larger absolute value of a coefficient means a stronger effect. This is a little tricky with re-sampling because the mean/SD of each 
mean_depth     <- mean(DS_WC_SP_RU$Depth,na.rm = T)                             #iteration will be slightly different on account of having slightly different values. Calculate the mean and SD of your complete data set 
sd_depth       <- sd(DS_WC_SP_RU$Depth,na.rm = T)                               #outside of the i-loop (DS_WC_SP_RU) and then using this to do the rescaling inside the i-loop (Data_i) 
...
N_reps <- 10000
Storage <- data.frame(Velocity = rep(NA,N_reps), Substrate = NA,                #Velocity_full = NA, Substrate_full = NA, Depth_full = NA,
                      Depth = rep(NA,N_reps),
...
Unique_Fish <- na.omit(unique(DS_WC_SP_RU$Tag))                               #Not sure the na.omit() is needed but it is good practice to remove NAs when using the unique() function to make sure they don't cause trouble.                                 #gives the number of individuals (not the total number of observations- doesn't count the same fish more than once)
...
for(i in 1:N_reps){                                                            #do 10,000 reps
  ...
  #Sample Used data (your_full_data ->DS_WC_SP_RU<- should be for one species, one site, one season, and (maybe) one mesohabitat) #The maybe will come into play when I look at the n\NumberOfIndividuals for each species/site/season/meso combo. If there are any with less than 10 unique individuals then I should combine mesohabs.
  
  Used_Data_i <- NULL
  for(j in 1:Number_Of_Individuals){                                            #create empty data frame
    #Ditto above on the na.omi(); it is good practice
    Fish_j <- na.omit(unique(DS_WC_SP_RU$Tag))[j]                               #randomly select one tag number, label it Fish j
    Fish_js_Rows <- DS_WC_SP_RU[which(DS_WC_SP_RU$Tag==Fish_j),]                #grabs Fish j's rows of data
  ...
  Avail_Data <- DS_WC_SP_RU[which(DS_WC_SP_RU$Presence == 0), ]               #Separate availabile data. y is Presence
  ...
  Storage$Velocity_25_Suit_Used[i] <- sum(HSC_Velocity$Suitable==1)
  Storage$Velocity_25_UnSuit_Used[i] <- sum(HSC_Velocity$Suitable==0)
  ...
  Storage$Substrate_25_Suit_Used[i] <- sum(HSC_Substrate$Suitable==1)
  Storage$Substrate_25_UnSuit_Used[i] <- sum(HSC_Substrate$Suitable==0)
  ...
  Storage$Depth_25_Suit_Used[i] <- sum(HSC_Depth$Suitable==1)
  Storage$Depth_25_UnSuit_Used[i] <- sum(HSC_Depth$Suitable==0)
...
write_csv(Storage, file = "Output/DS_WC_SP_RU_Results.csv")
...
#-0.1318219                         #avoiding fast water (negative)
...
# -0.4760467  0.1264710             #not significant   

# hist(Storage$Velocity_full)
# mean(Storage$Velocity_full, na.rm = TRUE)
# #-0.3746606                         #avoiding fast water (choosing slowest area within run)
# quantile(Storage$Velocity_full,c(0.025,0.975), na.rm = TRUE)
# #-0.8179007 -0.0570840              #significant

hist(Storage$Substrate)
mean(Storage$Substrate, na.rm = TRUE)
#0.1782062                          #favoring large substrate
quantile(Storage$Substrate,c(0.025,0.975), na.rm = TRUE)
#0.005086746 0.381657748            #doesn't cross 0 but is really near 0

# hist(Storage$Substrate_full)
# mean(Storage$Substrate_full, na.rm = TRUE)
# #0.2608193                          #same
# quantile(Storage$Substrate_full,c(0.025,0.975), na.rm = TRUE)
# #0.06488454 0.45593954              #significant and further from 0

hist(Storage$Depth)
mean(Storage$Depth, na.rm = TRUE)
# 0.6955118                         #selecting for depth within runs
quantile(Storage$Depth,c(0.025,0.975), na.rm = TRUE)
#0.4618695 0.8443504                #significant

# hist(Storage$Depth_full)
# mean(Storage$Depth_full, na.rm = TRUE)
# #0.7725609                          #same
# quantile(Storage$Depth_full,c(0.025,0.975), na.rm = TRUE)
# #0.4999513 0.9859653                #significant
...
hist(Storage$t_stat, breaks = 100)                   #Error in hist.default(Storage$t_stat, breaks = 100) : 'x' must be numeric
